...
export class DataConverterSignal {
  ...
  private async saveModelData(
    IfcProperties: IIfcProperties,
    coordinationMatrix: number[]
  ) {
    const {categories, uuid, ifcMetadata, properties, itemsByFloor} =
      IfcProperties;
    ...
    this._model.coordinationMatrix = new THREE.Matrix4().fromArray(
      coordinationMatrix
    );
  ...
  private async getBoundingBox() {
    this._bbox.reset();
    this.boxes = this._bbox.add(this._model);
  ...
  private createAllFragments(geometries: any) {
    ...
    for (const id in geometries) {
      ...
      const constructBuffer = this.constructBuffer(
        vertexData,
        indexData
      ...
      const fragment = new FRAGS.Fragment(
        constructBuffer,
        material,
        instances.length
      );
